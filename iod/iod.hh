...
namespace iod
{
  ...
  void apply_variables(F f, const iod_object<T...>& o)
  {
    return f(*static_cast<const T*>(&o)...);
  }

  template <typename F, typename... T>
  void apply_values(F f, const iod_object<T...>& o)
  {
  ...
  struct paste
  {
    ...
    static void run(iod_object<T...>& a,
                    const iod_object<U...>& b)
    {
  ...
  inline auto cat(const iod_object<T...>& a,
                  const iod_object<U...>& b)
  {
    return iod_object<T..., U...>(*static_cast<const T*>(&a)...,
