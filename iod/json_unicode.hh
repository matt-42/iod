...
namespace iod
{
  ...
  {
    return D(_append = [&s] (char c) { s.append(1, c); });
  }
  ...
  namespace unicode_impl
  {
    ...
    auto json_to_utf8(S&& s, T&& o)
    {
      ...
      if (s.get() != '"')
        throw std::runtime_error("json_to_utf8: JSON strings should start with a double quote.");
      ...
      while (true)
      {
        ...
        if (s.eof()) throw std::runtime_error("json_to_utf8: Unexpected end of string when parsing a string.");
        ...
        default:
          throw std::runtime_error("json_to_utf8: Bad JSON escaped character.");
        case '"': o.append('"'); break;
        case '\\': o.append('\\'); break;
        case '/': o.append('/'); break;
        case 'n': o.append('\n'); break;
      ...
      if (s.get() != '"')
        throw std::runtime_error("JSON strings must end with a double quote.");
        ...
        {        
          ...
          if (c1 < 0b11100000) // 11bits - 2 char: 110xxxxx	10xxxxxx
          {
            ...
            if (cp >= 0x0080 and cp <= 0x07FF)
              ...
              throw std::runtime_error("utf8_to_json: Bad UTF8 codepoint.");
          }
